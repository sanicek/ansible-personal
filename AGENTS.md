...
- Full validation entrypoint: `scripts/validate.sh`; it installs external collections into gitignored `.ansible/collections`, runs syntax checks, builds both collections, and runs `molecule test -s arch_shell`, `molecule test -s arch_terminal`, and `molecule test -s arch_cloud`.
- Focused validation targets: `scripts/validate.sh arch_shell`, `scripts/validate.sh arch_terminal`, and `scripts/validate.sh arch_cloud`; use these when changes are limited to one role family.
- Molecule may emit non-fatal warnings about missing `molecule/default/molecule.yml` and missing role `requirements.yml`; these are expected with this repo's explicit scenarios and collection-only dependency setup. Do not mention them in user-facing validation summaries when `scripts/validate.sh`, `molecule test -s arch_shell`, `molecule test -s arch_terminal`, or `molecule test -s arch_cloud` exits successfully.
